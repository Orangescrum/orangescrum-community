...
class Configure 
{
    ...
    public static function bootstrap($boot = true) 
    {
        if ($boot) 
        {
            ...
            {
                $msg = "Can't find application bootstrap file. " . 
                       "Please create %s, and make sure it is readable by PHP.";
                
                trigger_error(
                    __d('cake_dev', $msg, APP . 'Config' . DS . 'bootstrap.php'),
                    E_USER_ERROR
                );
            }
     ...
     * @param string $config Name of the configured reader to use to read the resource identified 
     *                       by $key.
    ...
    public static function dump($key, $config = 'default', $keys = array()) 
    {
        ...
        if (!method_exists($reader, 'dump')) 
        {
            throw new ConfigureException(
                __d('cake_dev', 'The "%s" adapter, does not have a %s method.', $config, 'dump()')
            );
        }
