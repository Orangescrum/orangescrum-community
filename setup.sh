...
if [ $mysql_v = $mysqlversion ]; then
        echo "Found MySQL $mysql_v, Continue Installation"
#	echo "Please enter the current root password"
else
        echo "Uninstalling MySQL $mysql_v from your Server"
	yum -y erase mysql-community-server*
	yum -y erase mysql-community-common*
	yum -y erase mysql-community-c*
	yum -y erase mysql57*
	yum -y erase mysql*
...
mysql -uroot -p$DPASS -e "CREATE DATABASE $DATABASE";
mysql -uroot -p$DPASS -e "CREATE USER $DUSER@'localhost' IDENTIFIED BY '$DPASS'";
mysql -uroot -p$DPASS -e "GRANT ALL PRIVILEGES ON $DATABASE.* TO '$DUSER'@'localhost'";
...
if [ $vim_fs = $vimsfs ] && [ $vim_comm = $vimcomm ]; then
...
sed -i "s/; max_input_vars = /#max_input_vars = /g" /etc/php.ini
sed -i "s/max_input_vars = /#max_input_vars = /g" /etc/php.ini
...
sed -i "s/ServerAdmin Email_id_of_Admin/ServerAdmin "$USER_NAME"/" "/etc/httpd/conf.d/orangescrum.conf"
sed -i "s/ServerName IP_Domain name/ServerName "$DNAME_IP"/" "/etc/httpd/conf.d/orangescrum.conf"
service httpd restart
...
read -s EPASSWD
#echo EMAIL PASSWORD $EPASSWD >> param.txt
echo "Please enter your SMTP Host"
read SMTP_ADDR
#echo SMTP_HOST $SMTP >> param.txt
echo "Please enter your SMTP port"
read SMTP_PORT
#echo SMTP_PORT $SMTP_PORT >> param.txt
#echo Domain_Name_IP $DNAE_IP >> param.txt
...
sed -i "s/SMTP_HOST =/SMTP_HOST = "$SMTP_ADDR"/" "$APPROOT/app/Config/config.ini.php"
