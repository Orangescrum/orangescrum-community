...
<table width="100%" class="tsk_tbl compactview_tbl">
    <tr style="" class="tab_tr">
        ...
        <td class="task_cn">
            <a href="javascript:void(0);" title="Task#" onclick="ajaxSorting(<%= '\'caseno\', ' + caseCount + ', this' %>);" class="sortcaseno">
                <div class="fl">Task#</div><div class="tsk_sort fl <% if(typeof csNum != 'undefined' && csNum != "") { %>tsk_<%= csNum %><% } %>"></div>
            </a>
        </td>
        <td class="task_wd">
            <a class="sorttitle" href="javascript:void(0);" title="Title" onclick="ajaxSorting(<%= '\'title\', ' + caseCount + ', this' %>);">
                <div class="fl">Title</div><div class="tsk_sort fl <% if(typeof csTtl != 'undefined' && csTtl != "") { %>tsk_<%= csTtl %><% } %>"></div>
            </a>
        </td>
        <td class="assign_wd_td">
            <a class="sortcaseAt" href="javascript:void(0);" title="Assigned to" onclick="ajaxSorting(<%= '\'caseAt\', ' + caseCount + ', this' %>);">
                <div class="fl">Assigned to</div><div class="tsk_sort fl <% if(typeof csAtSrt != 'undefined' && csAtSrt != "") { %>tsk_<%= csAtSrt %><% } %>"></div>
            </a>
        </td>
        <td class="tsk_due_dt">
            <a class="sortduedate" href="javascript:void(0);" title="Due Date" onclick="ajaxSorting(<%= '\'duedate\', ' + caseCount + ', this' %>);">
    ...
    <tr class="tr_all" id="curRow<%= caseAutoId %>">
        ...
        <td <% if(groupby =='' || groupby !='priority'){%>class="pr_<%= easycase.getPriority(casePriority) %>"<% } %> valign="top">
		    ...
		    <% } %>
                    ...
                    <li onclick="removeTask(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'\'' %>,<%= '\'' + projId + '\'' %>);" id="moveTask<%= caseAutoId %>" style=" <% if(caseFlag == 2){ %> display:block <% } else { %> display:none <% } %>">
                        <a href="javascript:void(0);"><div class="act_icon task_remove_mlst fl" title="Move Task To Milestone"></div>Remove from <br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Milestone</a>
                    </li>
                    <% } %>
                    <li class="divider"></li>
                    <% if(getdata.Easycase.isactive == 1){
            ...
            <% if(isactive==0){ %>
            <div id="csStsRep<%= count %>" class="fl tsk_sts"><div class="label new" style="background-color: olive">Archived</div></div>
        ...
        <td class="title_det_wd">
            ...
            <div class="fnt999 fl">
                <div class="fl">
                    <span id="stsdisp<%= caseAutoId %>" class="cview_hide"><% if(getTotRep && getTotRep!=0) { %>updated<% } else { %>created<% } %></span> by <span <% if(getdata.Easycase.usrName) { %> original-title="<%= getdata.Easycase.usrName %>" <% } %>><%= getdata.Easycase.usrShortName %></span>
