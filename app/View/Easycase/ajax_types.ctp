...
if(isset($typeArr))
{
	...
	foreach($typeArr as $typ)
	{
		...
		<li <?php if($t > 5){ $h++;?>id="hidType_<?php echo $h; ?>" style="display:none;"<?php } ?>>
			...
			<input type="checkbox" id="types_<?php echo $typeId; ?>" onClick="checkboxTypes('types_<?php echo $typeId; ?>','check');filterRequest('type');" style="cursor:pointer;" <?php if(strstr($CookieTypes,(string)$typeId)) { echo "checked"; } ?>/>
			<font class="filter-type-font" onClick="checkboxTypes('types_<?php echo $typeId; ?>','text');filterRequest('type');"  title="<?php echo $typeName; ?>">&nbsp;<?php echo "<img src='".HTTP_IMAGES."images/types/".$typeShortName.".png' /> ".$typeName; ?> (<?php if($proj_uniq_id != 'all'){ echo $typecount; }else{echo $typecount;}?>)</font>
