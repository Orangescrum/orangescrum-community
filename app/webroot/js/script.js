...
function trackLogin() 
{
    ...
    $.post(HTTP_ROOT + "users/session_maintain", {}, postCallback);
}
...
function saveAllFilters() 
{
    ...
    $.post(strURL + "ajax_save_filter", postData, postCallback);
}
...
function submitfilter() 
{
    ...
    if (filtername.trim() != "") 
    {
        ...
        $.post(strURL + "ajax_customfilter_save", postData, postCallback);
    } 
    else 
    {
        $(".eror_txt").show();
...
function getMemeberList() 
{
    ...
    $.post(url + "projects/member_list", postCallback, 'json');
}
...
function restoreTaskDetail(caseid, caseNo) 
{
    if (confirm("Are you sure you want to restore task #" + caseNo + " ?")) 
    {
        ...
        $.post(HTTP_ROOT + "archives/move_list", postData, postCallback);
    }
}
...
function editask(csuid, projUid, projName) 
{
    ...
    if (csuid) 
    {
        ...
        var postCallback = function (res) {
            ...
            var nestedPostCallback = function (data) {
                if (data) 
                {
            ...
            $.post(HTTP_ROOT + "easycases/ajax_quickcase_mem", nestedPostData, nestedPostCallback);
        };
        
        $.post(HTTP_ROOT + 'easycases/edit_task_details', postData, postCallback, 'json');
    }
}


// add assignto, user checkboxs,
function case_quick(easycase) 
{ 
        ...
        for (ipusr in PUSERS) 
        {
            for (ipusr1 in PUSERS[ipusr]) 
            {
                ...
                (
                    easycase 
                    && easycase.assign_to 
                    && easycase.assign_to == pusr.User.id 
...
function view_project_menu(page) 
{
    ...
    if ($('#ajaxViewProjects').html() == "") 
    {
        ...
        else
        {
            ...
            $.post(usrUrl + "users/project_menu", postData, postCallback);
        }
    }
    else
    {
...
function savecategorytab() 
{
    ...
    $.post(HTTP_ROOT + "users/ajax_savecategorytab", postData, postCallback);
}
...
$(function () {
    ...
    $('#file_upload1').fileUploadUI(fileUploadUiConfig);
});
...
function doneCropImage() 
{
    ...
    if (width != 0 && height != 0 && imgName.trim() != '') 
    {
        ...
        $.post(HTTP_ROOT + "users/done_cropimage", postData, postCallback);
    }
}
//Profile image ends
...
$(".icon-edit-usr").click(function () {
    ...
    $.post(HTTP_ROOT + "projects/ajax_edit_project", postData, postCallback);
});
...
function EditTaskTemp(tempId, tasktempname, pagenum) 
{
    ...
    $.post(HTTP_ROOT + "templates/ajax_add_task_template", postData, postCallback, 'json');
}
...
function submitProject(proj, shrt) 
{
    ...
    $.post(HTTP_ROOT + "projects/ajax_check_project_exists", postData, postCallback);
    return false;
}
...
$(".icon-remove-usr").click(function () {
    ...
    $.post(HTTP_ROOT + "projects/user_listing", postData, postCallback);
});
...
function searchremuserkey() 
{
    ...
    if (project_id) 
    {
        ...
        $.post(strURL1, postData, postCallback);
    }
}
...
function ajaxCaseSearch() 
{
    ...
    $.post(url + "ajax_search", postData, postCallback);
}
...
function searchFile(file_id, uniq_id, file_srch) 
{
    ...
    $.post(fileUrl, postData, postCallback);
...
function add_project(usr_id, usr_name, is_invite_user) 
{
    ...
    $.post(HTTP_ROOT + "users/add_project", postData, postCallback);
}
...
$(".icon-remprj-usr").click(function () {
    ...
    $.post(HTTP_ROOT + "users/project_listing", postData, postCallback);
});
...
function grantOrRemoveModerator(obj) 
{
    ...
    if (confirm("Are you sure you want to " + msg + " moderator to '" + usr_name + "'?")) 
    {
        ...
        $.post(HTTP_ROOT + "users/grant_moderate", postData, postCallback);
    }
}
...
function resend_invitation(qrst, email) 
{
    if (confirm('Are you sure you want to Resend Invitation email to \'' + email + '\' ?')) {
        ...
        $.post(HTTP_ROOT + 'users/resend_invitation', postData, postCallback, 'json');
    }
    else
    {
        return false;
...
function postSupport() 
{
    ...
    else
    {
        ...
        else
        {
            ...
            $.post(HTTP_ROOT + "users/post_support_inner", postData, postCallback);
        }
    }
...
function showTemplates(id, name)
{
    ...
    if (id != "New")
    {
        ...
        $.post(HTTP_ROOT + "easycases/ajax_case_template", postData, postCallback);
    }
}
...
function check_proj_size() 
{
    if ($('#add_new_popup').is(":visible")) 
    {
        ...
        $.post(sizeUrl + "ajax_check_size", postData, postCallback);
    }
}
...
function search_project_easypost(val, e) 
{
    ...
    if (e.keyCode == 40 || e.keyCode == 38) 
    {
        if (key == 40)  // Down key
        {
            if 
            (
                !$('#' + menu_div_id + ' > a').length 
                || $('#' + menu_div_id + '> a').filter('.popup_selected').is(':last-child')
            ) 
            {
        ...
        else if (key == 38) // Up key
        {
            ...
            (
                !$('#' + menu_div_id + ' > a').length 
                || $('#' + menu_div_id + '> a').filter('.popup_selected').is(':first-child')
            ) 
            {
                $current = $('#' + menu_div_id + ' > a').last('a');
            } 
    ...
    else 
    {
        ...
        if (val != "") 
        {
            ...
            $.post(strURL + "search_project_menu", postData, postCallback);
        } 
        else 
        {
            $('#ajaxaftersrchc').hide();
...
function removeFile(id, div, storage) 
{
    ...
    $.post(strURL + "fileremove", postData, postCallback);
        ...
        if (confirm("Are you sure you want to remove?")) 
        {
            ...
            var postCallback = function (data) {
                if (data) 
                {
                    ...
                    $.post(url, postData, postCallback);
                }
            };
...
function notifyMe(title, desc, icon) 
{
    ...
    else if (Notification.permission !== 'denied') 
    {
        ...
        var requestCallback = function (permission) {
            //alert(permission);
        ...
        Notification.requestPermission(requestCallback);
    }
...
function addEditMilestone(obj, mileuniqid, mid, name, cnt, mlstfrom) 
{
    ...
    $.post(HTTP_ROOT + "milestones/ajax_new_milestone", postData, postCallback);
}
...
function delMilestone(obj, name, uniqid) 
{
    ...
    if (confirm("Are you sure you want to delete milestone '" + name + "' ?")) 
    {
        ...
        $.post(loc, postData, postCallback, 'json');
    }
    return false;
}
...
function addTaskToMilestone(obj, mstid, projid, cnt) 
{
    ...
    $.post(HTTP_ROOT + "milestones/add_case", postData, postCallback);
}
...
function assignCaseToMilestone(el) 
{
    ...
    if (done) 
    {
        ...
        $.post(HTTP_ROOT + 'milestones/assign_case', postData, postCallback);
    } 
    else 
    {
...
function view_project_milestone() 
{
    ...
    $.post(HTTP_ROOT + "milestones/ajax_milestone_menu", postData, postCallback);
}
...
function caseMilestone(pjid, pname, page) 
{
    ...
    if (pjid) 
    {
        ...
        $.post(HTTP_ROOT + "milestones/milestone", postData, postCallback);
    }
}
//Milestone ends
...
showMilestoneList = function (mlstpaginate, isActive, pointer, search_key) {
        ...
        if (res) 
        {
            ...
            $(".kanban-child .kbtask_div").live("hover", function (obj) {
                ...
                (
                    (
                        $(this).is(":last-child") 
                        || $(this).is(":nth-last-child(3)") 
                        || $(this).is(":nth-last-child(2)")
                    ) 
                    && (parseInt(curindex) > 1) 
                    && ($(this).parents(".jspPane").height() > 400)
                )
                {
                    var clickCallback = function (cobj) {
                    ...
                    $(this).find('.dropdown').on('click', clickCallback);
                }
            });
            ...
            $("div [id^='set_due_date_']").each(function (i) {
                var onSelectCallback = function (dateText, inst) {
                    ...
                    $.post(HTTP_ROOT + "easycases/ajax_change_DueDate", postData, postCallback, 'json');
                };
    ...
    $.post(mURL, postData, postCallback);
...
$(function () {
    var config = {
        ...
        stop: function () {
            ...
            $.post(HTTP_ROOT + "easycases/ajax_save_dashboard_order", postData, postCallback);
        }
    };
...
function loadDashboardPage(projid) 
{
    ...
    if 
    (
        getCookie('DASHBOARD_ORDER') 
        && $.inArray('7', getCookie('DASHBOARD_ORDER').split('::')[1].split(',')) === -1
    ) 
    {
...
function loadSeqDashboardAjax(sequency, projid) 
{
    ...
    $.post(url + action, postData, successCallback);
}
...
function iniChartTskProgress(id, res) 
{
    ...
    $("#" + id).highcharts(highChartsConfig);
}
...
function deleteAssignedProject(id, userId, name, isInvite) 
{
    if (id) 
    {
        ...
        if (conf == true) 
        {
            ...
            $.post(strURL + "ajax_assignedproject_delete", postData, successCallback);
        } 
        else 
        {
            return false;
...
function deleteUsersInProject(userId, projectId, name) 
{
    if (userId) 
    {
        ...
        if (conf == true) 
        {
            ...
            $.post(strURL, postData, successCallback);
        } 
        else 
        {
            return false;
...
function removeTaskFromMilestone(obj) 
{
    ...
    $.post(HTTP_ROOT + "milestones/removeCasesFromMilestone", postData, successCallback);
}
...
function removecaseFromMilestone(obj) 
{
    if (confirm('Are you sure you want to remove selected tasks from Milestone?')) 
    {
        ...
        if (done) 
        {
            ...
            $.post(HTTP_ROOT + 'milestones/remove_case', postData, successCallback);
        } 
        else 
        {
...
function milstoneonTask(mlstname, mlstid) 
{
    ...
    $.post(HTTP_ROOT + 'milestones/milestone_list', postData, successCallback, 'json');
}
...
function calendarView(type) 
{
    ...
    if 
    (
        (type == 'hash' && urlHash == 'calendar') 
        || (type == 'calendar' && urlHash == 'calendar') 
        || (filterV == 'calendar' && type == 'calendar')
    ) 
    {
        $('#calendar_view').hide();
        ...
        $.post(tskURL, postData, function (res) {
            $('#calendar_view').show();
