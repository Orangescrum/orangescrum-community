...
function filterProjectSearch(id,type)
{
    var proj_srch = $("#"+id).val();
...
$("#show_lpanel").click(function(){ 
    $("#left_panel").animate({marginLeft:"0px",opacity:"show"},"slow",function(){                                                                                                  });
...
$(window).scroll(function(){
    ...
    else
    {
        ...
        if 
        (
            (win_ht <  parseInt(new_popup_ht+80)) 
            || (parseInt(win_ht-40) <  new_popup_ht_opt4) 
            || (parseInt(win_ht-40) <  new_popup_ht_opt5)
        )
        {
...
function submitfilter(){
    ...
    if (filtername.trim() != "")
    {
        ...
        $.post(strURL + "ajax_customfilter_save", postData, function(data){
            if (data) 
            { 
                if (data == 'success')
                {
...
function filterValue(id,prjuniqid,date,type,status,member,assignto,praiority,searchtxt,pname)
{
...
function submitsubscription()
{
    ...
    if 
    (
        (!storage.value.match(numbers) && storage.value.trim() != "Unlimited")  
        || storage.value.trim() == "" || storage.value.trim() == 0
    ) 
    {
    ...
    (
        (!project_limit.value.match(numbers) && project_limit.value.trim() != "Unlimited") 
        || project_limit.value.trim() == "" 
        || project_limit.value.trim() == 0
    ) 
    {
    ...
    (
        (!user_limit.value.match(numbers) && user_limit.value.trim() != "Unlimited") 
        || user_limit.value.trim() == "" 
        || user_limit.value.trim() == 0
    ) 
    {
    ...
    (
        (!milestone_limit.value.match(numbers) && milestone_limit.value.trim() != "Unlimited") 
        || milestone_limit.value.trim() == "" 
        || milestone_limit.value.trim() == 0
    )
    {
