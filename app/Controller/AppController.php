...
class AppController extends Controller
{
    ...
    public function beforeFilter()
    {
        ...
        if
        (
            Configure::read('default_action') == 'mydashboard' 
            && isset($_COOKIE['DEFAULT_PAGE']) 
            && in_array($_COOKIE['DEFAULT_PAGE'],array('dashboard','mydashboard'))
        ) 
        {
        ...
        if
        (
            $this->params['controller'] == 'easycases' 
            && $this->params['action'] == 'dashboard' 
            && isset($this->params->query['case'])
        ) 
        {
        ...
        $ajaxPageArray = array(
            ...
            'caseview_remove',
            'project_all',
            'jquery_multi_autocomplete_data',
            'search_project_menu',
            'project_listing',
            'assign_prj',
            'contactnow',
        ...
        if
        (
            isset($_COOKIE['USER_UNIQ']) 
            && isset($_COOKIE['USERTYP']) 
            && isset($_COOKIE['USERTZ'])
        ) 
        {
        ...
        if ($this->Auth->User("id"))
        {
            ...
            if
            (
                (!stristr(PAGE_NAME,"ajax_") && !in_array(PAGE_NAME,$ajaxPageArray)) 
                || PAGE_NAME=='categorytab' 
                || PAGE_NAME =='ajax_savecategorytab'
            ) 
            {
                $User = ClassRegistry::init('User');
                $User->unbindModel(array('hasAndBelongsToMany' => array('Project')));
                $userDeskNotify = $User->find(
                    'first', array(
                        'conditions' => array(
            ...
            if
            (
                PAGE_NAME != 'image_thumb' 
                && PAGE_NAME != 'project_menu' 
                && PAGE_NAME != 'search_project_menu' 
                && PAGE_NAME != 'ajax_case_menu' 
                && !in_array(PAGE_NAME, array('recent_projects','recent_milestones','statistics','usage_details','task_progress','leader_board'))
            ) 
            {
            ...
            if (SES_TYPE == 3)
            {
                if
                (
                    (CONTROLLER == "users" && PAGE_NAME == "manage") 
                    || (CONTROLLER == "users" && PAGE_NAME == "add_new") 
                    || (CONTROLLER == "users" && PAGE_NAME == "add_template") 
                    || (CONTROLLER == "users" && PAGE_NAME == "manage_template")
                ) 
                {
            ...
            (
                ...
                || PAGE_NAME == "activity" 
                || PAGE_NAME == "glide_chart" 
                || PAGE_NAME == 'chart' 
                || PAGE_NAME == 'hours_report' 
            ...
            (
                isset($_REQUEST['case']) 
                && urldecode(trim($_REQUEST['case'])) 
                && isset($_REQUEST['project'])
                && urldecode(trim($_REQUEST['project'])) 
                && !isset($_GET['search']) 
            ...
            if (PAGE_NAME == "download")
            {
                ...
                $getCaseId = $this->CaseFile->find(
                    'first', 
                    array('conditions' => array('CaseFile.file' => $filename),
                    'fields' => array('CaseFile.easycase_id'))
                );
                
                if
                (
                    isset($getCaseId['CaseFile']['easycase_id']) 
                    && $getCaseId['CaseFile']['easycase_id']
                ) 
                {
                    $caseid = $getCaseId['CaseFile']['easycase_id'];
                    $this->loadModel('Easycase');
                    
                    $getProj = $this->Easycase->find(
                        'first', 
                        array('conditions'=>array('Easycase.id' => $caseid,'Easycase.isactive'=>1), 
                        'fields' => array('Easycase.project_id'))
                    );
            ...
            if (PAGE_NAME == "downloadImgFile")
            {
                ...
                $getCaseId = $this->CaseFile->find(
                    'first', 
                    array(
                        'conditions' => array('CaseFile.file' => $filename), 
                        'fields'     => array('CaseFile.easycase_id')
                    )
                );
            ...
            (
                ...
                && PAGE_NAME != 'search_project_menu' 
                && PAGE_NAME != 'ajax_case_menu' 
                && !in_array(PAGE_NAME,array('to_dos','recent_projects','recent_activities','recent_milestones','statistics','task_progress','leader_board','ajax_activity'))
            ) 
            {
                if (!stristr(PAGE_NAME, "ajax_") && !in_array(PAGE_NAME, $ajaxPageArray))
                {
            ...
            )
            {
                ...
                if
                (
                    $this->params['action'] == 'upgrade_member' 
                ...
                elseif (SES_TYPE <= 3)
                {
                    ...
                    (
                        ...
                        && (!$prjcnt) 
                        && (PAGE_NAME != 'help') 
                        && (PAGE_NAME != 'default_inner') 
                        && (PAGE_NAME != 'hide_default_inner') 
                        && (PAGE_NAME != 'ajax_new_project') 
                        && (PAGE_NAME != 'launchpad') 
                        && (PAGE_NAME != 'googleConnect') 
                        && (PAGE_NAME != 'ajax_check_project_exists') 
                        && (PAGE_NAME != 'check_fordisabled_user') 
                        && (PAGE_NAME != 'onbording') 
                        && (PAGE_NAME != 'ajax_quickcase_mem') 
                        && (PAGE_NAME != 'ajax_case_menu') 
                        && (PAGE_NAME != 'ajax_project_size') 
                        && (PAGE_NAME != 'member_list') 
                        && (PAGE_NAME != 'ajax_check_size') 
                        && (PAGE_NAME != 'new_user') 
                        && (PAGE_NAME != 'getProjects') 
                        && (PAGE_NAME != 'add_project')
                        && (PAGE_NAME != 'logout') 
                        && (PAGE_NAME != 'ajax_check_user_exists') 
                        && (PAGE_NAME != 'image_thumb') 
                        && (PAGE_NAME != 'ajax_recent_case') 
                        && (PAGE_NAME != 'ajax_custom_filter_show') 
                        && (PAGE_NAME != 'post_support_inner')
                        && (PAGE_NAME != 'session_maintain') 
                        && 
                        (
                            CONTROLLER != 'users' 
                            ||
                            (
                                PAGE_NAME != 'account_activity'
                                && PAGE_NAME != 'transaction' 
                                && PAGE_NAME != 'creditcard' 
                                && PAGE_NAME != 'subscription' 
                                && PAGE_NAME != 'profile' 
                                && PAGE_NAME != 'changepassword' 
                                && PAGE_NAME != 'email_notifications' 
                                && PAGE_NAME != 'show_preview_img' 
                                && PAGE_NAME != 'done_cropimage' 
                                && PAGE_NAME != 'pricing' 
    ...
    function betauser_limitation()
    {
        ...
        if
        (
            $limitation['UserSubscription']['subscription_id'] == 1 
            || $limitation['UserSubscription']['subscription_id'] == 9
        ) 
        {
        ...
        if
        (
            isset($limitation['UserSubscriptions']['storage_limit']) 
            && 
            ( 
                (strtolower($limitation['UserSubscriptions']['storage_limit'])=='unlimited') 
                || $limitation['UserSubscription']['is_free']
            )
        ) 
        {
        ...
        (
            isset($limitation['UserSubscriptions']['storage_limit']) 
            && $usedspace<= $limitation['UserSubscriptions']['storage_limit']
        ) 
        {
    ...
    function projcetcount($company_id = SES_COMP, $sub_limitation = array())
    {
        if (!$sub_limitation)
        {
            ...
            $sub_limitation = $usersubscription->find(
                'first',
        ...
        if
        (
            $sub_limitation['UserSubscription']['project_limit'] 
            && (strtolower($sub_limitation['UserSubscription']['project_limit'])=='unlimited' 
            || $sub_limitation['UserSubscription']['is_free'])
        ) 
        {
            return 'Unlimited';
        }
    ...
    function usercount($company_id = ACC_ID, $sub_limitation = array())
    {
        ...
        if
        (
            $sub_limitation['UserSubscription']['user_limit'] 
            && (strtolower($sub_limitation['UserSubscription']['user_limit'])=='unlimited' 
            || $sub_limitation['UserSubscription']['is_free'])
        ) 
        {
            return 'Unlimited';
