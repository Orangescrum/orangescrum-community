        ...
        if($ProjId && $curCaseNo) {
            ...
            foreach($allMemsArr as $k=>$getAllMems) {
                ...
                unset(
                        ...
                        $allMemsArr[$k]['User']['istype'],
                        $allMemsArr[$k]['User']['short_name'],
                        $allMemsArr[$k]['User']['uniq_id']
                );
	...
	$hjk = fopen('ttyu.txt','a');
        ...
        if(intval($oauth_return)) {
            foreach($sqlcasedata as $key=>$csdata) {
                ...
                unset(
                        $sqlcasedata[$key]['Easycase']['uniq_id'],
                        $sqlcasedata[$key]['Easycase']['user_id'],
                        $sqlcasedata[$key]['Easycase']['project_id']
                );
            }
        }
        //add mahavir
        ...
        if($caseTypeRep == 10 || $caseLegendRep == 3 || $caseLegendRep == 5) {
            ...
            if($caseDuDate != "NULL" && $caseDuDate != "0000-00-00" && $caseDuDate != "" && $caseDuDate != "1970-01-01") {
                ...
                if(strpos($csDuDtFmt,'Today'))$csDuDtFmt = 'Due '.$csDuDtFmt;
                else $csDuDtFmt = 'Due On '.$csDuDtFmt;
            }
            else {
                $csDuDtFmtT = '';
                $csDuDtFmt = '';
            ...
            if($caseDuDate != "NULL" && $caseDuDate != "0000-00-00" && $caseDuDate != "" && $caseDuDate != "1970-01-01") {
                if($caseDuDate < $curdtT) {
                    ...
                    $csDuDtFmtT = $dt->facebook_datestyle($caseDuDate);
                    $csDuDtFmt = $dt->dateFormatOutputdateTime_day($caseDuDate,$curDateTz,'week');
                    if(strpos($csDuDtFmt,'Today')!==false)$csDuDtFmt = '<span class="due-txt">Due '.$csDuDtFmt.'</span>';
                    else $csDuDtFmt = '<span class="due-txt">Due On '.$csDuDtFmt.'</span>';
                }
            }
            else {
                $csDuDtFmtT = '';
...
//		
            ...
            $bugCase = $this->Easycase->query('SELECT COUNT(DISTINCT Easycase.id) as bgcscount FROM easycases Easycase WHERE Easycase.isactive=1 AND Easycase.type_id = 1 AND Easycase.istype= 1 AND Easycase.project_id='.$proj_id." ".$qry.$searchcase);
...
//			),'fields' => 'DISTINCT Easycase.id'));
            ...
            $bugCase = $this->Easycase->query('SELECT COUNT(DISTINCT Easycase.id) as bgcscount FROM easycases Easycase WHERE Easycase.isactive=1 AND Easycase.type_id = 1 AND Easycase.istype= 1 AND Easycase.project_id IN('.$idlist.") ".$qry.$searchcase);
        ...
        //$resCaseMenu['cs'] = $case;
        $resCaseMenu['bugCase'] = $bugCase[0][0]['bgcscount'];
    ...
    function ajax_case_status() {
        ...
        $types_sql =  "select DISTINCT t.name,t.id,t.short_name,(select count(Easycase.id) from easycases as Easycase".$mlstnQ1." where Easycase.istype='1' AND Easycase.type_id=t.id AND Easycase.isactive='1' ".$mlstnQ2.$projQry." ".trim($qry).") as count from types as t 
