...
class EasycasesController extends AppController
{
    ...
    function help()
    {
        ...
        if ($getSearchResult && count($getSearchResult) > 0)
        { 
            # If search result data present then display results
            $this->set("isSearchresult", 1);
            $this->set("allHelpData", $getSearchResult);
        }
        else
        {
            # If no data present then display no data message
        ...
        if (isset($caseAll) && !empty($caseAll))
        {
            foreach ($caseAll as $key => $getdata)
            {
                ...
                (
                    ...
                    || SES_TYPE == 2 
                    || (
                        $getdata['Easycase']['legend'] == 1 
                        && SES_ID == $getdata['Easycase']['user_id']
                    )
                )
                {
    ...
    function case_project()
    {
        ...
        if ($commonAllId)
        {
            ...
            $email_notification = array('allfiles' => $allfiles, 'caseNo' => $caseStsNo, 'closeStsTitle' => $closeStsTitle, 'emailMsg' => $emailMsg, 'closeStsPid' => $closeStsPid, 'closeStsPri' => $closeStsPri, 'closeStsTyp' => $closeStsTyp, 'assignTo' => $assignTo, 'usr_names' => $usr_names, 'caseuniqid' => $caseuniqid, 'csType' => $emailType, 'closeStsPid' => $closeStsPid, 'caseStsId' => $caseStsId, 'caseIstype' => 5, 'caseid_list' => $caseid_list, 'caseUniqId' => $closeStsUniqId);
    ...
    function exportTaskcsv()
    {
        ...
        $csv_output .= "Tasks#,Title,Description,Status,Type,Milestone,Assigned To,Due Date,Created By,Date Created,Last Updated\n";
        ...
        if (isset($this->request->data['check_csv']))
        {
            if (isset($this->request->data['check_typ']) && (trim($this->request->data['check_typ']) == 'printcsv'))
            {
                ...
                foreach ($milestone as $key => $val)
                {
                    ...
                    $this->Easycase->recursive = -1;
                    $result = $this->Easycase->query($sql);
    ...
    function ajax_common_breadcrumb()
    {
        ...
        if ($case_status && $case_status != "all")
        {
            ...
            if (strstr($case_status, "-"))
            {
                ...
                foreach ($expst as $st)
                {
                    $status .= "<div class='fl filter_opn' rel='tooltip' title='Task Status' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"status\");'>" . $this->Format->displayStatus($st) . "<a href='javascript:void(0);' onclick='common_reset_filter(\"taskstatus\",\"" . strrev($st) . "\",this);' class='fr'>X</a></div>";
                }
            }
            else
            {
        ...
        if ($case_types && $case_types != "all")
        {
            ...
            if (strstr($case_types, "-"))
            {
                ...
                foreach ($expst3 as $st3)
                {
                    ...
                    $types.= "<div class='fl filter_opn' rel='tooltip' title='Task Type' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"types\");'>" . $csTypArr['Type']['short_name'] . "<a href='javascript:void(0);' onclick='common_reset_filter(\"tasktype\",\"" . $st3 . "\",this);' class='fr'>X</a></div>";
                }
                $types = trim($types, ', ');
            }
            else
            {
                ...
                $types = "<div class='fl filter_opn' rel='tooltip' title='Task Type' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"types\");'>" . $csTypArr['Type']['short_name'] . "<a href='javascript:void(0);' onclick='common_reset_filter(\"tasktype\",\"" . $case_types . "\",this);' class='fr'>X</a></div>";
            }
        ...
        if (!empty($date) && ($date != 'any'))
        {
            ...
            {
                $arr['date'] = "<div class='fl filter_opn' rel='tooltip' title='Time' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"date\");'>" . str_replace(":", " - ", $date) . "<a href='javascript:void(0);' onclick='common_reset_filter(\"date\",\"\",this);' class='fr'>X</a></div>";
            }
        }
        else
        {
        ...
        if (!empty($duedate))
        {
            ...
            {
                $arr['duedate'] = "<div class='fl filter_opn' rel='tooltip' title='Due Date' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"duedate\");'>" . str_replace(":", " - ", $duedate) . "<a href='javascript:void(0);' onclick='common_reset_filter(\"duedate\",\"\",this);' class='fr'>X</a></div>";
            }
        }
        else
        {
    ...
    function save_editedvalue()
    {
        ...
        (
            (
                ...
                || SES_TYPE == 2 
                || SES_TYPE == 3 
                || ($thisCase['Easycase']['legend'] == 1 && SES_ID == $thisCase['Easycase']['user_id'])
            ) 
            && $thisCase['Easycase']['message']
        )
        {
            $canEdit = 1;
        }
    ...
    function ajax_customfilter_save()
    {
        ...
        $this->loadModel('CustomFilter');
        $customFilters = $this->CustomFilter->find('first', array('conditions' => array('CustomFilter.filter_name' => $filterName, 'CustomFilter.company_id' => SES_COMP, 'CustomFilter.user_id' => SES_ID), 'fields' => array('CustomFilter.id')));
    ...
    /**
     * Used for the kanban view of the tasks
...
CompanyUser.company_id=" . SES_COMP . " $usercond) AS total_users $projectcond";
    ...
    /**
     * Create downloadable folder which will contain a .csv File and a Folder containg all the Attachment
