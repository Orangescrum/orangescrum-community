...
class ReportsController extends AppController 
{
    ...
    /**
     * Weekly usage Report for admin and owner only
     * @return HTML html page with usage details
     */
    public function weeklyusage_report()
    {
        if (SES_TYPE > 2)
        {
        ...
        $user_details = $companyusercls->find(
            'all',
            array(
                'joins' => array(
                    array(
                ...
                'fields' => "Company.id,DATE(Company.created) AS dt_created,User.timezone_id,User.id,User.name,User.last_name,User.email,Company.name,Company.seo_url"
            )
        );
