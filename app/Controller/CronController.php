...
class CronController extends AppController
{
    ...
    function email_notification()
    {
        ...
        foreach ($getAllNot as $usr)
        {
            ...
            if 
            (
                $usr['UserNotification']['value'] == 1 
                || ($usr['UserNotification']['value'] == 2 && $day == "Fri") 
                || ($usr['UserNotification']['value'] == 3 && $lastDate == GMT_DATE)
            )
            {
        ...
        foreach ($getAllNot as $emlNot)
        {
            ...
            if ($hour == 6 && $min < 30)
            {
                ...
                $arr4 = array(
                    "OR" => array(
                        'AND' => array(
                            'Easycase.assign_to' => $uid
                        ), 
                        array(
                            'Easycase.assign_to' => '0', 
                            'Easycase.user_id' => $uid
                        )
                    )
                );
                
                # Late cases
                $case_late = $Easycase->find(
                    'all', 
                    array(
                        'conditions' => array(
                            'Easycase.isactive' => 1, 
                            'Easycase.assign_to' => $usr['User']['id'], 
                            'Easycase.due_date !=' => NULL, 
                            'Easycase.project_id' => $projArr, 
                            'Easycase.istype' => 1, 
                            'Easycase.type_id !=' => 10, 
                            'Easycase.legend NOT' => array(5, 3), 
                            $arr_late, 
                            $arr4
                        ), 
                        'fields' => array(
                ...
                if (count($case_cming) || count($case_late))
                {
                    ...
                    if (count($case_cming) != 0)
                    {
                        ...
                        $typ_arr = array(
                            '1' => '<font color="#763532">New</font>', 
                            '2' => '<font color="#244F7A">In Progress</font>', 
                            '3' => '<font color="#387600">Closed</font>', 
                            '4' => '<font color="#55A0C7">Start</font>', 
                            '5' => '<font color="#EF6807">Resolved</font>'
                        );
    ...
    function dailyupdate_notifications()
    {
        ...
        $getAllNot = $DailyupdateNotification->find(
            'all', 
            array(
                'conditions' => array(
            ...
            if ($dateSent != $dateToday && $nottime <= $curtime)
            {
                ...
                foreach ($projArr as $prj)
                {
                    ...
                    if ($query_All)
                    {
                        ...
                        $typ_arr = array(
                            '1' => '<font color="#AE432E">New</font>', 
                            '2' => '<font color="#244F7A">In Progress</font>', 
                            '3' => '<font color="#387600">Closed</font>', 
                            '4' => '<font color="#244F7A">Started</font>', 
                            '5' => '<font color="#EF6807">Resolved</font>'
                        );
    ...
    function dailyUpdateMail()
    {
        ...
        if (isset($allData) && !empty($allData))
        {
            foreach ($allData as $key => $value)
            {
                ...
                if ($value['DailyUpdate']['cron_email_date'] != $dt['0'])
                {
                    # Send an email once a day. Check current date
                    ...
                    if (in_array(date('l', strtotime($locDT)), $days))
                    {
                        # Check weekdays.
    ...
    /**
     *  Sends weekly usage of the company with respect to the project
     */
    ...
    /**
     * It will check the .zip file and delete those file which are created b4 2days.
     * @return bool 
     */
    ...
    /**
     * It will check the .zip file and delete those file which are created b4 24 hours.
     * @return bool 
     */
    ...
    /**
     *  It will check the temp file and delete those file which are created b4 24 hours.
     * @return bool 
     */
