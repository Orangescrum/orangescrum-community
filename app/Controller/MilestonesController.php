...
class MilestonesController extends AppController
{
    ...
    function delete_milestone($uniqid = '', $page = NULL)
    {
        ...
        if (isset($uniqid) && $uniqid)
        {
            ...
            (
                count($checkMstn) 
                && isset($checkMstn[0]['Milestone']['id']) 
                && $checkMstn[0]['Milestone']['id']
            )
            {
    ...
    function ajax_new_milestone($mileuniqid = null)
    {
        ...
        if
        (
            !empty($_REQUEST['data']['Milestone']) 
            && $_REQUEST['data']['Milestone']['title'] 
            && !$this->request->data['mileuniqid']
        )
        {
    ...
    /**
     * Manage milestone listing
     * @author GDR
     * @return json 
     */
    public function manage_milestone()
    {
    ...
    function milestone($type = NULL)
    {
        ...
        (
            $type == "completed" 
            || (isset($this->data['mlsttype']) && $this->data['mlsttype'] == 0)
        )
        {
        ...
        elseif ($_COOKIE['ALL_PROJECT'] == 'all')
        {
