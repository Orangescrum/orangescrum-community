...
class User extends AppModel
{
    ...
    function get_billing_info($account_id = SES_COMP)
    {
        ...
        $user_sub = $usersub->find(
            'first', 
            array(
                'conditions' => array('company_id' => $account_id), 
                'order' => 'id DESC'
            )
        );
        ...
        $userlist = $this->find('all', array('joins' => array(
                array(
                    ...
                    'conditions' => array(
                        ...
                        '(CompanyUser.is_active = 1 OR CompanyUser.is_active=2)'
                    )
                )
            ),
    ...
    /**
     *  Checking limitation of the user while upgrading the plan
     * @return array
     */
    function downgrade_limitation($subscription = array())
    {
        if ($subscription)
        {
            ...
            $totalUsers = $companyusers_cls->find(
                'count', 
                array(
                    'conditions' => array('company_id' => SES_COMP, 'is_active !=' => 3)
                )
            );
    ...
    function getProjectOwnAdmin()
    {
        $query = 
            ...
            "FROM "
                . "users AS User,"
                . "company_users AS CompanyUser " . 
            "WHERE "
                . "User.id=CompanyUser.user_id "
                . "AND CompanyUser.company_id='" . SES_COMP . "' "
                . "AND CompanyUser.is_active ='1' "
                . "AND CompanyUser.user_type!='3' "
                . "AND User.isactive='1' " . 
            "ORDER BY CompanyUser.user_type ASC";
    ...
    /**
     * Check email and URL existance with our db
     * @return array 
     */
    function validate_emailurl($data = array())
    {
        ...
        if ($data['email'])
        {
            if (filter_var($data['email'], FILTER_VALIDATE_EMAIL))
            {
                $checkUsr = $this->find(
                    'first', 
                    array(
                        'conditions' => array('User.email' => urldecode($data['email'])), 
        ...
        {
            $checkUsr = $Company->find(
                'first', 
                array(
                    'conditions' => array(
                        'Company.seo_url' => $seo_url
                    ), 
        ...
        if (isset($data['coupon_code']) && $data['coupon_code'])
        {
            ...
            $coupon = $Coupon->find(
                ...
                array(
                    'conditions' => array(
                        'Coupon.code' => trim($data['coupon_code']), 
                        'Coupon.isactive' => '1', 
                        array(
                            'OR' => array(
                                'Coupon.expires >= CURDATE()', 
                                'Coupon.expires' => '0000-00-00'
                            )
                        )
                    )
                )
